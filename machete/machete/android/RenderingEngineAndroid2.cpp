...
#include "RenderingEngineAndroid2.h"

#ifdef TARGET_ANDROID

RenderingEngineAndroid2::RenderingEngineAndroid2(IResourceManager *rm) {
    resMan = rm;
}

ivec2 RenderingEngineAndroid2::GetScreenSize() const { return size; }

void RenderingEngineAndroid2::Initialize(int width, int height) {
    vtxBind = -1;
    lastBind = -1;
    size.x = width;
    size.y = height;
...
void RenderingEngineAndroid2::Clear() {
    glClear(GL_COLOR_BUFFER_BIT);
    primCount = 0;
    vtxCount = 0;
}

void RenderingEngineAndroid2::Draw(vec2 pos, vec2 size, float rotation, float scale, vertuv *verts, unsigned int texId) {
...
void RenderingEngineAndroid2::Draw() {
...
GLuint RenderingEngineAndroid2::CreateBuffer(vertuv* verts) const {
    GLuint buff;
...
void RenderingEngineAndroid2::DeleteBuffer(GLuint buffer) const {
    glDeleteBuffers(1, &buffer);
}

Tex RenderingEngineAndroid2::CreateTexture(const char* texture) {
    Tex tTex;
...
void RenderingEngineAndroid2::ApplyOrtho(float maxX, float maxY) const {
    float ortho[16] = {
        2.0f / maxX, 0, 0, 0,
        0, 2.0f / maxY, 0, 0,
        0, 0, -1, 0,
...
GLuint RenderingEngineAndroid2::BuildProgram(const char* vertexShaderSource, const char* fragmentShaderSource) const {
...
GLuint RenderingEngineAndroid2::BuildShader(const char* source, GLenum shaderType) const {
...
void RenderingEngineAndroid2::CreateBuffers() {
    unsigned int vtx = 0;
